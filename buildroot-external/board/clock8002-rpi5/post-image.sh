...
BOOT_RUNTIME_FILES="alsa-ltc alsa-ltc_cmd.sh alsa-ltc_pokemon.sh sdl-clock clock_cmd.sh clock_pokemon.sh clock-bridge clock_bridge_cmd.sh clock_bridge_pokemon.sh DejaVuSans.ttf nano"
	...
	for init_override in \
		...
		if [ -f "${GOLDEN_DIR}/etc/init.d/${init_override}" ]; then
			cp -f "${GOLDEN_DIR}/etc/init.d/${init_override}" \
				"${BINARIES_DIR}/init.d/${init_override}"
		fi
	done
...
if [ -d "${BINARIES_DIR}/piclock" ] && [ -f "${BOOT_IMG}" ]; then
	...
	rm -rf "${BINARIES_DIR}/piclock/init.d"
	mkdir -p "${BINARIES_DIR}/init.d"
	...
	if [ -d "${BINARIES_DIR}/init.d" ]; then
		MTOOLS_SKIP_CHECK=1 mmd -i "${BOOT_IMG}" ::init.d 2>/dev/null || true
		for f in "${BINARIES_DIR}"/init.d/*; do
			[ -f "${f}" ] || continue
			MTOOLS_SKIP_CHECK=1 mcopy -o -i "${BOOT_IMG}" "${f}" ::init.d/
		done
	fi
