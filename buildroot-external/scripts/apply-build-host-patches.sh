#!/bin/sh
# Apply manual build-host patches to a Buildroot source tree.
#
# The Mesa upgrade path below is only for the Buildroot 2025.02-era tree that
# shipped Mesa 24.0.9. Newer Buildroot releases already carry a newer Mesa with
# matching hashes, so forcing the 25.0.7 hash file there breaks downloads.
...
BUILDROOT_VERSION="$(sed -n 's/^export BR2_VERSION := //p' "$BUILDROOT/Makefile" | head -n 1)"

echo "Applying build-host patches to: $BUILDROOT"
[ -n "$BUILDROOT_VERSION" ] && echo "Detected Buildroot version: $BUILDROOT_VERSION"
...
MESA3D_VERSION="$(sed -n 's/^MESA3D_VERSION = //p' "$MESA3D_MK" | head -n 1)"

if [ "$MESA3D_VERSION" = "24.0.9" ] || [ "$MESA3D_VERSION" = "25.0.7" ]; then
    # 1. Version bump
    if [ "$MESA3D_VERSION" = "24.0.9" ]; then
...
EOF
    ...
    echo "  mesa3d: detected version ${MESA3D_VERSION:-unknown}; skipping 2025.02-specific Mesa patch path"
fi
