...
- Active code: `v4/` only. It is the sole Go module and the single source of the product.
- `v4` is NOT a version number. It comes from the upstream Go module path `gitlab.com/clock-8001/clock-8001/v4` and is unrelated to this repository's `v1.x` tag line.
...
- The primary release artifact is the **installer tarball** (`piClock-v1.x.y-linux-arm64.tar.gz`), produced by `make release` in `v4/`. This is what gets attached to GitHub releases.
