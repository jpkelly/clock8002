...
def main():
    ...
    soak_p.add_argument("--screen-name", default=None,
                         help="run this soak in a detached screen session with "
                              "this name. The session ends cleanly when the soak "
                              "completes (no lingering shell is left behind). "
                              "If not already inside a screen session, the harness "
                              "re-executes itself under screen and exits.")
    ...
    cycle_p.add_argument("--force", action="store_true",
                          help="deploy even if another run is marked 'running' "
                               "against this host (see check_no_active_run)")
    cycle_p.add_argument("--screen-name", default=None,
                          help="run this cycle test in a detached screen session "
                               "with this name. The session ends cleanly when the "
                               "test completes (no lingering shell is left behind). "
                               "If not already inside a screen session, the harness "
                               "re-executes itself under screen and exits.")
    ...
    # Optional self-daemonize under a detached screen session. The old ad-hoc
    # launch pattern (screen -dmS <name> bash -lc "... harness.py run ...; exec
    # bash") left a lingering shell alive after the run finished because of the
    # trailing `exec bash`. Running the harness itself under screen (without any
    # shell tail) means the screen session ends cleanly the moment the harness
    # exits. Only the soak/cycle run modes support --screen-name.
    screen_name = getattr(args, "screen_name", None)
    if screen_name and not os.environ.get("STY"):
        # We are not already inside screen. Re-exec ourselves under screen.
        # Rebuild argv dropping --screen-name AND its value (so the inner
        # invocation runs directly and can't re-enter this branch).
        ...
        for a in sys.argv[1:]:
            ...
            if skip:
                ...
                continue
            argv.append(a)
        screen_cmd = ["screen", "-dmS", screen_name] + argv
        print(f"launching run in screen session '{screen_name}': "
              f"{' '.join(screen_cmd)}")
        subprocess.run(screen_cmd)
        print(f"session '{screen_name}' started; monitor with "
              f"screen -r {screen_name}")
        sys.exit(0)
