...
* Network
  * Fix HDMI info overlay reporting "Network Mode: Unknown". The overlay code in
    `clock/engine.go` was reading the wrong boot-partition path for the network
    config. It now reads the correct path, so static, DHCP, and dual modes are
    reported correctly.
  * Implement `mode=dual`. `piclock-network.sh` now keeps DHCP on the wired
    connection and adds the configured static address as a secondary IP,
    matching the behaviour already documented in `network.ini.default` and
    `boot-README.txt`.
  * Correct the stale path comment in `network.ini.default` to point to the
    boot-partition location (`/boot/firmware/piclock/network.ini`).
* OLED
  * In `mode=dual`, the OLED IP line now alternates every 5 seconds between the
    ...
    single stable IP.
* Release packaging
  * Rename the Trixie installer tarball from
...
* Verified by a clean install on a fresh Trixie reflash plus a 15h soak
  (825 samples, zero errors, stable RSS, no throttling).
